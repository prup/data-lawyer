...
/**
 * Models a relation with a set of output columns.
 * 
 * @author prasang
 * 
 */
public abstract class Relation {
	...
	private final HashMap<Column, ColumnOptInfo> _optinfos;
	...
	public Relation(String name, Counters counters) throws DataLawyerException {
		...
		_optinfos = new HashMap<Column, ColumnOptInfo>();
	}

	public final void appendColumn(Column c, ColumnOptInfo optinfo) {
		if (!columns.contains(c)) {
			columns.add(c);
			_optinfos.put(c, optinfo);
		}
	}
	
	public final void deleteColumn(Column c) throws DataLawyerException {
		if (!columns.contains(c))
			throw new DataLawyerException("Column being deleted does not exist: "
					+ c.toString());
		columns.remove(c);
		_optinfos.remove(c);
	}
		...
		for (Column c: columns) {
			if (c.getInfo().equalsByTree(columnTree)) {
				if (toRet == null)
					...
					throw new DataLawyerException("Ambiguous column: " + toRet + " vs " + c  + " while parsing column: " + columnTree);
			}
		}
		if (toRet == null) {
			MyLogger.getLog().info("This column not found: " + columnTree.toString());
	...
	public Redundancy getRedundancy(Column c) throws DataLawyerException {
		if (hasColumn(c))
			...
			throw new DataLawyerException("Column not found: " + c + " in relation: "
					+ _name);
	}
	...
	public final void projectOut() throws DataLawyerException {
		...
		if (hasProjectableColumn
			...
			throw new DataLawyerException(
					"Can not perform a project on: " + this.getClass().getSimpleName());
		...
		if (hasProjectableColumn) {
			...
			if (_parent == null) {
				...
				for (Column c: columns)
					prOp.appendColumn(c, new ColumnOptInfo(this.getColumnOptInfo(c)));
			...
			for (Column c : columns)
				if (prOp.hasColumn(c)
						&& getColumnOptInfo(c)._redundancy.equals(Redundancy.True)) {
					prOp.deleteColumn(c);
					if (_parent != null)
	...
	protected void addOptColInfo(Column c, ColumnOptInfo colOptInfo) {
		if (_optinfos.containsKey(c))
			return;
		_optinfos.put(c, colOptInfo);
	}

	public final ColumnOptInfo getColumnOptInfo(Column c) {
		if (c.getInfo() instanceof ConstColumn)
			return new ColumnOptInfo();
		if (!_optinfos.containsKey(c)) 
			throw new IllegalArgumentException("Column: " + c.toString() + " not found in column set: " + _optinfos.keySet().toArray().toString());
		return _optinfos.get(c);
	}
	...
	public final void reorderColumn(Column c, int newPos) {
		if (c.getInfo() instanceof DerivedColumn) {
			...
			_optinfos.put(c, new ColumnOptInfo());
		} else {
			columns.remove(c);
			columns.add(newPos, c);
		}
...
//		additionalInfo += "OptColInfo: " + _optinfos.keySet().toString() + "\n";
